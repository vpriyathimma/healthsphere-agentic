...
export class AgentCoreStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: AgentCoreStackProps) {
    ...
    // Wire each agent to the OAuth credential provider it uses to authenticate to
    // the gateway (M2M). This is CDK-owned so it survives every redeploy — nothing
    // is set by hand. The value is only the non-secret provider NAME; the secret is
    // in the token vault and the agent->provider binding is the IAM policy on the role.
    // environments is keyed by agent name (see AgentCoreApplication.environments).
    const gatewayProviderByAgent: Record<string, string> = {
      healthsphereagents: 'healthsphere-supervisor-gw',
      records: 'healthsphere-records-gw',
      admissions: 'healthsphere-admissions-gw',
    };
    for (const [agentName, env] of this.application.environments.entries()) {
      const provider = gatewayProviderByAgent[agentName];
      if (provider) {
        env.runtime.addEnvironmentVariable('GATEWAY_PROVIDER_NAME', provider);
      }
    }
